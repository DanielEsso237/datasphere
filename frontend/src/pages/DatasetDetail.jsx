...
import {
  ...
  rateDataset, getComments, addComment, deleteComment, deleteDataset
} from '../services/api'
import StarRating from '../components/StarRating'
import Spinner from '../components/Spinner'
import { Download, Trash2, User, Calendar, Tag, BarChart2, Send } from 'lucide-react'
...
export default function DatasetDetail() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Recharger le dataset pour avoir le vrai compteur à jour
  ...
  const handleRate = async (score) => {
    ...
    try {
      ...
      // Recharger le dataset pour avoir avg_rating et ratings_count à jour
      const { data } = await getDataset(id)
      setDataset(data)
    ...
    <div className="page-container detail-page">
      ...
      <div className="detail-header">
        <div className="detail-meta-top">
          ...
          <span className="card-filetype" style={{ background: fileColor }}>
        ...
        <div className="detail-info-row">
          <span>
            <User size={15} />
            <Link to={`/profile/${dataset.uploaded_by?.username}`}>
              {dataset.uploaded_by?.username}
            </Link>
          </span>
          <span><Calendar size={15} /> {date}</span>
          <span>{dataset.file_size_display}</span>
          <span><Download size={15} /> {dataset.download_count} téléchargement{dataset.download_count !== 1 ? 's' : ''}</span>
        </div>
        <div className="detail-rating-row">
          <StarRating value={Math.round(dataset.avg_rating)} readonly />
          <span>
            {dataset.avg_rating} / 5 ({dataset.ratings_count} note{dataset.ratings_count !== 1 ? 's' : ''})
          </span>
        </div>
            ...
            {user && !isOwner && (
              <div className="rate-section">
                ...
                <p className="rate-hint">
                  {myRating ? `Vous avez noté ${myRating}/5` : 'Cliquez pour noter'}
                </p>
              </div>
            )}
          </div>
            ...
            {preview?.columns ? (
              <>
                <p className="preview-info">
                  <BarChart2 size={15} />
                  {preview.columns.length} colonnes · {preview.rows?.length} lignes affichées
                </p>
